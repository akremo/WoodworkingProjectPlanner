...
- **Boards** (and `scrap`) are strip stock: parts are ripped into lanes across the board's width and crosscut within a lane; kerf applies between lanes and between crosscuts. A lane's width is set by its first piece; narrower parts can join. A part is only placed in an orientation that keeps it entirely inside the board — one longer than the board is left for the next board. Boards are a fallback target for sheet-type parts and vice-versa when a matching thickness is available.
...
- **Waste** is attributed per stock item against its own reporting base: planed boards report against the physical source (the thickness difference is sawdust and counts), while a resawn layer or exact board reports against the stock actually cut into. A resawn board is not waste — each layer contributes only its own leftover, and a board with no parts on it reports no waste at all.
