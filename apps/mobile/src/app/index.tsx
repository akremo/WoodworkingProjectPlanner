    ...
    <ScrollView className="flex-1 bg-stone-50" keyboardShouldPersistTaps="handled">
      <View className="p-4">
        <View className="flex-row items-baseline justify-between">
          <Text className="text-2xl font-bold text-stone-900">Part List</Text>
          <Pressable onPress={onExport} className="rounded-lg border border-stone-300 bg-white px-3 py-1.5">
            <Text className="text-xs font-medium text-stone-700">Export CSV</Text>
          </Pressable>
        </View>
        <Text className="mt-1 text-sm text-stone-500">Editable — saved on this device.</Text>

        {parts.map((p, index) => (
          <View key={p.id} className="mt-4 rounded-xl bg-white p-4 shadow-sm">
        ...
        <Pressable
          onPress={add}
          className="mt-4 items-center rounded-xl border border-dashed border-stone-400 py-3">
          <Text className="font-medium text-stone-600">+ Add part</Text>
        </Pressable>

        <View className="mt-4 rounded-xl bg-amber-100 p-4">
          <View className="flex-row justify-between">
            <Text className="font-semibold text-stone-900">Total (all parts)</Text>
            <Text className="font-semibold text-stone-900">{totalBdFt.toFixed(2)} bd-ft</Text>
          </View>
          <View className="mt-1 flex-row justify-between">
            <Text className="font-semibold text-stone-900">Total price</Text>
